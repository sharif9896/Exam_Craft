      ...
      >
        ...
        <nav className="mt-6 px-4 space-y-1.5 overflow-y-auto h-[calc(100vh-100px)] custom-scrollbar">
          {menuItems.map((item, idx) => {
            const isSubItemActive = item.submenu?.some(sub => sub.path === location.pathname);
                  ...
                  <>
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 group ${
                        isMenuOpen || isSubItemActive ? 'bg-slate-800 text-white' : 'hover:bg-slate-800/50 hover:text-white'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <span className={`${isMenuOpen || isSubItemActive ? 'text-blue-400' : 'text-slate-400 group-hover:text-blue-400'}`}>
                          {item.icon}
                        </span>
